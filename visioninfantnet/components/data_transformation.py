...
from visioninfantnet.entity.artifact_entity import DataTransformationArtifact, DataValidationArtifact
...
# ============================================================================
# YAMNET EMBEDDINGS (placeholder for now)
# ============================================================================
...
class DataTransformation:
    ...
    def _extract_split_base_features(
        ...
        """
        For each row (audio file) in df, compute:

        - Mel spectrogram (n_mels=64), store list for later padding
        - PNG spectrogram image in spectrogram_image_dir
        - Image array (224, 224, 3)
        - ResNet50 embedding (2048,)
        - PANNs embedding (2048,)
        - YAMNet embedding (1024,)
        - eGeMAPS (88,)
        - MFCC LLDs (frames, 20) -> stored in a list for BoAW/FV

        Returns a dict with numpy arrays + list of MFCC LLDs.
        """
        ...
        try:
            ...
            logging.info(f"Extracting base features for split with {len(df)} files...")
